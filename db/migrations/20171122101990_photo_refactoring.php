...
class PhotoRefactoring extends AbstractMigration
{
	public function change() {
	  ...
	  $table
		...
		->addColumn('datetime_create', 'timestamp', ['comment' => 'Дата добавления этого варианта размер-кол-во', 'default' => 'CURRENT_TIMESTAMP'])
	  ;
	  ...
	  foreach ($this->fetchAll('SELECT * FROM photo_price_history') as $row) {
		$rowPhoto = $this->fetchRow("SELECT * FROM photo WHERE photo_size_id = {$row['photo_size_id']} AND count = {$row['count']}");
