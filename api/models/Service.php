...
class Service extends Model
{
	...
	public static function getTodayCash()
	{
		...
		$query = "select SUM(service_price_history.price) as summ from service_sale
					JOIN service_price_history ON service_price_history.service_id = service_sale.service_id AND service_sale.datetime >= service_price_history.datetime_from AND (service_sale.datetime < service_price_history.datetime_to OR service_price_history.datetime_to IS NULL)
 AND service_sale.department_id = service_price_history.department_id
WHERE service_sale.datetime::date = now()::date AND service_sale.department_id = $department_id";
